...
class BasicChatPage extends StatefulWidget {
  ...
  @override
  _BasicChatPageState createState() => _BasicChatPageState();
}
  ...
  Future<void> _initializeAgent() async {
    ...
    try {
      ...
      final AgentConfig config = AgentConfig(
        ...
        turnDetection: const TurnDetectionConfig(
      ...
      body: Column(
        children: <Widget>[
          ...
          Container(
            ...
            child: Row(
              children: <Widget>[
                Icon(_isLoading ? Icons.hourglass_empty : Icons.check_circle,
                    color: Colors.white,),
...
class _VoiceChatPageState extends State<VoiceChatPage> {
  ...
  Future<void> _initializeAgent() async {
    ...
    try {
      ...
      _session!.state.listen((final SessionStatus SessionStatus final state) {
        setState(() {
...
class ToolsDemoPage extends StatefulWidget {
  ...
  @override
  _ToolsDemoPageState createState() => _ToolsDemoPageState();
}

class _ToolsDemoPageState extends State<ToolsDemoPage> {
  late VoiceAgent _agent;
  AgentSession? _session;
  final List<Message> _messages = <Message>[];
  final List<String> _availableTools = <String>['Weather', 'Calculator', 'Time'];
  ...
  Future<void> _initializeAgent() async {
    ...
    try {
        ...
        FunctionTool(
          ...
          parameters: <String, dynamic>{
            ...
            'properties': <String, Map<String, Object>>{
              'location': <String, String>{'type': 'string', 'description': 'City name'},
              'unit': <String, Object>{'type': 'string', 'enum': <String>['celsius', 'fahrenheit']},
            },
        ...
        FunctionTool(
          ...
          parameters: <String, dynamic>{
            ...
            'properties': <String, Map<String, String>>{
              'expression': <String, String>{'type': 'string', 'description': 'Math expression'},
            },
          ...
          function: (final Map<String, dynamic> args) async {
            ...
            try {
              // This is a simple simulation - in real app you'd use a proper calculator
              if (expression.contains('+')) {
                final List<String> parts = expression.split('+');
                final int result = int.parse(parts[0].trim()) + int.parse(parts[1].trim());
                return <String, Object>{'result': result, 'expression': expression};
              } else if (expression.contains('*')) {
                final List<String> parts = expression.split('*');
                final int result = int.parse(parts[0].trim()) * int.parse(parts[1].trim());
                return <String, Object>{'result': result, 'expression': expression};
              } else {
                return <String, Object>{'result': 0, 'expression': expression, 'error': 'Unsupported operation'};
              }
            } catch (e) {
              return <String, Object>{'result': 0, 'expression': expression, 'error': 'Invalid expression'};
            }
          },
        ),
        FunctionTool(
          ...
          parameters: <String, dynamic>{
            'type': 'object',
            'properties': <String, Map<String, String>>{
              'timezone': <String, String>{'type': 'string', 'description': 'Timezone (e.g., America/New_York)'},
            },
      ...
      _session!.events.listen((final AgentEvent event) {
        ...
        } else if (event is ToolExecutedEvent) {
          setState(() {
            _messages.add(Message(
              id: 'tool_${DateTime.now().millisecondsSinceEpoch}',
              role: MessageRole.system,
              content: '🔧 Used tool: ${event.toolName}\nResult: ${event.result}',
              timestamp: DateTime.now(),
            ),);
          });
        }
      ...
      print('Error initializing tools agent: $e');
      setState(() {
        _messages.add(Message(
          id: 'error',
          role: MessageRole.system,
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                const SizedBox(height: 8),
                const Text(
                  'Try asking: "What\'s the weather in New York?" or "What\'s 15 + 27?"',
                ...
                ? Center(
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        const SizedBox(height: 16),
                        Text(
                          'Tools Demo\n\nAsk me to use tools like weather or calculator!',
                          textAlign: TextAlign.center,
                          style: TextStyle(color: Colors.grey[600]),
                        ),
                      ],
            ...
            _SettingsCard(
              ...
              children: <Widget>[
                ListTile(
                  ...
                  subtitle: const Text('Configure speech recognition and synthesis'),
                  trailing: const Icon(Icons.arrow_forward_ios),
                  onTap: () {},
                ),
                ListTile(
